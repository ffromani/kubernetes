...
func TestDevicesToAllocateConflictWithUpdateAllocatedDevices(t *testing.T) {
	...
	activePods := []*v1.Pod{
		{ObjectMeta: metav1.ObjectMeta{UID: types.UID(podToAllocate)}},
	}
	testManager := &ManagerImpl{
		endpoints:             make(map[string]endpointInfo),
		healthyDevices:        make(map[string]sets.Set[string]),
...
func TestReservationLossWindowDoesNotDuplicateDeviceAcrossPods(t *testing.T) {
	...
	// 1) pod1 reserves a device and makes it unavailable;
	...
	//
	// TODO: Complement this regression test with focused coverage for:
	// 1) GetPreferredAllocation failures after a partial reservation;
	// 2) Allocate failures after a reservation has been recorded;
	// 3) reserved entries staying out of checkpoint/runtime-facing behavior.
